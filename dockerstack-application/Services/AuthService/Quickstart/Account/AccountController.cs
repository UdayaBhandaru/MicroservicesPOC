...
namespace IdentityServer4.Quickstart.UI
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(
            ...
            IEventService events,LdapUserManager ldapUserManager)
        {
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var result = await _signInManager.CheckPasswordSignInAsync(user, model.Password, model.RememberLogin);
                if (result.Succeeded)
                {
                    var userId = await _userManager.GetUserIdAsync(user);
                    var userName = await _userManager.GetUserNameAsync(user);
                    var id = new ClaimsIdentity("Identity.Application");
                    id.AddClaim(new Claim(JwtClaimTypes.Name, userId));
                    id.AddClaim(new Claim(JwtClaimTypes.Subject, user.Id));
                    id.AddClaim(new Claim("SessionId", Guid.NewGuid().ToString()));
        ...
        public async Task<IActionResult> ExternalLogin(string provider, string returnUrl)
        {
            ...
            {
                // start challenge and roundtrip the return URL and cls
        ...
        private async Task<IActionResult> ProcessWindowsLoginAsync(string returnUrl)
        {
            ...
            {
                var user=await this._ldapUserManager.FindByNameAsync(result.Principal.Identity.Name);
                ...
                id.AddClaim(new Claim("email_id", user.Email));
                id.AddClaim(new Claim(ClaimTypes.NameIdentifier, user.Email));
            ...
            FindUserFromExternalProviderAsync(AuthenticateResult result)
        {
            ...
            // find external user
            var user = await this._userManager.FindByNameAsync(result.Principal.Identity.Name); 

            return (user, provider, providerUserId, claims);
        }

        private async Task<(IdentityUser user, string provider, string providerUserId, IEnumerable<Claim> claims)>
            FindUserFromWindows(AuthenticateResult result)
        {
            var externalUser = result.Principal;
            ...
            // find external user
            var user = await this._ldapUserManager.FindByNameAsync(userIdClaim.Value);
